...
export const TourCard = ({ image, title, location, duration, price, rating, reviewCount, maxGroup = 12, featured = false }: TourCardProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="group bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-500 border border-border/50">
